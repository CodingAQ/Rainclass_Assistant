...
这是一个用于雨课堂的自动化工具，旨在帮助学生自动完成随堂测试和签到。该工具通过模拟浏览器操作，结合单个 AI 模型或多 AI 并行投票自动识别题目并给出答案，同时支持微信通知提醒。

## 功能特性

- **自动答题**: 自动识别并提交客观题；主观题暂时只返回题目类型并通知，不自动提交。
- **AI模型支持**: 支持接入多种 AI，以及支持多 AI 同时答题。
- **静默运行**: 支持无头模式，可在后台静默运行，不显示浏览器界面。
...
点击**获取登录Cookies**按钮。程序将启动一个浏览器窗口，在该窗口中完成长江雨课堂的登录。登录成功后，Cookies将自动保存。
...
1. **首次运行**: 
   - 运行 `python main.py` 或双击文件 `main.pyw`  启动程序。
   - 切换到“设置”页面。
